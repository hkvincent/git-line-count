...
# The next parameter is the branch name
branch_name=$1
shift

# Optional merge base, defaulting to master if not specified
merge_base=${1:-master}

# Fetch all commit hashes from the specified range
commit_hashes=$(git rev-list $(git merge-base $merge_base $branch_name)..$branch_name)
...
for commit_hash in $commit_hashes; do
  ...
  while read -r add del file; do
    # Skip if add or del are not numbers
